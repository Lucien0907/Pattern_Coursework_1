...
for p in range(Mpca):
    ...
    for l in range(Mlda):
        ...
        for i in range(test_size):
           ...
           if y_train[e_idx] == y_test[i]:
               mark+=1
        recog_acc_mt[l,p] = (mark/test_size)*100

ax = sns.heatmap(recog_acc_mt, cmap="Blues", square = True, xticklabels = 5, yticklabels = 5)

ax.axhline(y=0, color='k',linewidth=5)
ax.axvline(x=0, color='k',linewidth=5)
ax.axhline(y=Mlda, color='k',linewidth=5)
ax.axvline(x=Mpca, color='k',linewidth=5)
plt.show()